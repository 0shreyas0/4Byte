...
export const NavLinks: React.FC<NavLinksProps> = ({
  ...
  const containerClass = surfaceScheme === 'secondary' ? 'bg-secondary' : 'bg-background';
  const pillClass = surfaceScheme === 'secondary' ? 'bg-background' : 'bg-secondary';

  return (
    <div
      ref={containerRef}
      className={`hidden md:flex items-center rounded-full p-1 mr-4 relative ${containerClass}`}
    >
      ...
      <AnimatePresence initial={false}>
        {pillStyle && (
          <motion.span
            key={layoutId}
            className={`absolute top-1 bottom-1 rounded-full shadow-sm pointer-events-none z-0 ${pillClass}`}
