...
export const useSearch = (maxResults: number, localItems?: Item[]) => {
  ...
  useEffect(() => {
    ...
    // If local items are provided, perform immediate fuzzy search
    ...
    // Otherwise, perform debounced API search
    debounceTimerRef.current = setTimeout(async () => {
      ...
      try {
        ...
        if (!abortController.signal.aborted && Array.isArray(data)) {
          // Apply additional fuzzy ranking to API results if helpful
          const ranked = data
            .map(item => ({
              item,
